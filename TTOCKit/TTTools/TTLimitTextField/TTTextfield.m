...
{
    if (self.TTTextFieldReturnTypeBlock) {
        self.TTTextFieldReturnTypeBlock(self);
    }
    return YES;
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if (self.TTTextFieldEndEditBlock) {
        self.TTTextFieldEndEditBlock(self);
    }
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (_isOnlyNumber) {
        ...
        if ([string TT_is:TTTextFieldStringTypeNumber]) {
    ...
    if (!_isSpecialCharacter) {
        if ([self.canInputCharacters containsObject:string] ||[self.canInputPasswords containsObject:string]) {
            ...
            if ([string TT_isSpecialLetter] || [self.canotInputCharacters containsObject:string]) {
                return NO;
            }
            return YES;
        }
            ...
            if ((single >='0' && single<='9') || single=='.')//数据格式正确
            {
                ...
                else
                {
                    if (isHaveDian)//存在小数点
                    {
                        ...
                        int compareNum = 2;
                        if (self.afterDotNum>0) {
                            compareNum = self.afterDotNum;
                        }
...
- (void)TT_addTargetEditingChanged
{
...
- (void)textFieldTextEditingChanged:(id)sender
{
    ...
    //所有都处理完了来回调
    if (self.TTTextfieldTextChangedBlock) {
        self.TTTextfieldTextChangedBlock(self);
    }
}
...
-(void)setupLimits:(NSString *)toBeString
{
    ...
    if (!_isSpecialCharacter) {
        ...
        self.text = [toBeString TT_removeSpecialLettersExceptLetters:filterArrs];
    ...
    if (_isOnlyNumber) {
        if ([toBeString TT_is:TTTextFieldStringTypeNumber]) {
    ...
    if (_maxCharactersLength > 0) {
        if (!_isPhoneNumber) {  //电话号码时_maxCharactersLength无效
            int totalCountAll = [toBeString TT_getStrLengthWithCh2En1];
            if (totalCountAll > _maxCharactersLength) {
                ...
                for (int i = 0; i < toBeString.length; i++) {
                    ...
                    BOOL currentIsCN = [str1 TT_is:TTTextFieldStringTypeChinese]; //当前字符是不是中文
                    if (currentIsCN) {
                        totalCount +=2;
                    }else{
                        totalCount +=1;
                    }
#warning TT -2016--10-09
                    //点击过快，会替换到最后一个字符串？？？为啥？？
