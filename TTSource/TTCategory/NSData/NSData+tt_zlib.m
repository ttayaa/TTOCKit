...
#import <zlib.h>
#import <dlfcn.h>
#import "NSData+tt_zlib.h"

static const uInt CHUNK_SIZE = 65536;

NSString *const tt_ZlibErrorDomain = @"se.bitba.tt_ZlibErrorDomain";
NSString *const tt_ZlibErrorInfoKey = @"zerror";

@implementation NSData (tt_zlib)
static void *tt_zlibOpen()
{
    static void *libz;
...
- (NSData *)tt_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    ...
    [self tt_inflate:^(NSData *toAppend) {
...
- (NSData *)tt_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self tt_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
            error:error];
    return outData;
...
- (BOOL)tt_inflate:(void (^)(NSData *))processBlock
          error:(NSError *__autoreleasing *)error
{
    void *libz = tt_zlibOpen();
...
//    int (*deflateEnd)(z_streamp) = (int (*)(z_streamp))dlsym(libz, "deflateEnd");
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:tt_ZlibErrorDomain
                                                code:tt_ZlibErrorCodeInflationError
                                            userInfo:@{tt_ZlibErrorInfoKey : @(ret)}];
        return NO;
    }
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)tt_deflate:(void (^)(NSData *))processBlock
          error:(NSError *__autoreleasing *)error
{
    void *libz = tt_zlibOpen();
//    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
...
//    int (*inflateEnd)(z_streamp) = (int (*)(z_streamp))dlsym(libz, "inflateEnd");
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:tt_ZlibErrorDomain
                                                code:tt_ZlibErrorCodeDeflationError
                                            userInfo:@{tt_ZlibErrorInfoKey : @(ret)}];
        return NO;
    }
    Bytef *source = (Bytef *)[self bytes]; // yay
...
- (BOOL)tt_writeDeflatedToFile:(NSString *)path
                          error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = tt_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self tt_deflate:
                   ^(NSData *toAppend) {
...
static NSFileHandle *tt_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        if (!success) {
            if (error) *error = [NSError errorWithDomain:tt_ZlibErrorDomain
                                                    code:tt_ZlibErrorCodeCouldNotCreateFileError
                                                userInfo:nil];
            return nil;
        }
    }
