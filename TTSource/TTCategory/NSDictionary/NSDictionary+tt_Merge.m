...
#import "NSDictionary+tt_Merge.h"

@implementation NSDictionary (tt_Merge)
/**
    ...
    [resultTemp enumerateKeysAndObjectsUsingBlock: ^(id key, id obj, BOOL *stop) {
        if ([dict1 objectForKey:key]) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict1 objectForKey: key] tt_dictionaryByMergingWith: (NSDictionary *) obj];
        ...
        {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict2 objectForKey: key] tt_dictionaryByMergingWith: (NSDictionary *) obj];
                [result setObject: newVal forKey: key];
            } else {
                [result setObject: obj forKey: key];
            }
...
- (NSDictionary *)tt_dictionaryByMergingWith:(NSDictionary *)dict {
    return [[self class] tt_dictionaryByMerging:self with: dict];
}
