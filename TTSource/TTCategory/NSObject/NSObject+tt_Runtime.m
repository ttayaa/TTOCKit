...
#import "NSObject+tt_Runtime.h"
#import <objc/runtime.h>
BOOL tt_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void tt_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void tt_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (tt_Runtime)

+ (void)tt_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    tt_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)tt_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    tt_method_append(self.class, klass, newMethod);
}

+ (void)tt_replaceMethod:(SEL)method fromClass:(Class)klass
{
    tt_method_replace(self.class, klass, method);
}

- (BOOL)tt_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class tt_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)tt_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)tt_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass tt_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)tt_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
