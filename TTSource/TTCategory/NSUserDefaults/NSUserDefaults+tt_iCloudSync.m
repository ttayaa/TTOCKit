...
#import "NSUserDefaults+tt_iCloudSync.h"


@implementation NSUserDefaults (tt_iCloudSync)
...
- (void)tt_removeValueForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    [self tt_removeObjectForKey:key iCloudSync:sync];
}



-(void)tt_setObject:(id)value forKey:(NSString *)defaultName iCloudSync:(BOOL)sync
{
    if (sync)
...
- (void)tt_removeObjectForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    if (sync)
