...
#import "UIButton+tt_MiddleAligning.h"

@interface UIImage (tt_MiddleAligning)

@end

@implementation UIImage (tt_MiddleAligning)

- (UIImage *)tt_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
@implementation UIButton (tt_MiddleAligning)

- (void)tt_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    if (imageSize.width > ceilf(maxImageWidth)) {
        ...
        newImage = [self.imageView.image tt_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
        imageSize = newImage.size;
    }
    if (imageSize.height > ceilf(maxImageHeight)) {
        CGFloat ratio = maxImageHeight / imageSize.height;
        newImage = [self.imageView.image tt_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
        imageSize = newImage.size;
    }
    if (newImage) {
