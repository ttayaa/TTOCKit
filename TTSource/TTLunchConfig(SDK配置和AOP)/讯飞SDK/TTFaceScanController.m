...
#import "TTFaceScanController.h"
#import <AVFoundation/AVFoundation.h>
#import <AssetsLibrary/AssetsLibrary.h>
#import <QuartzCore/QuartzCore.h>
#import "TTPermissionDetector.h"
#import "UIImage+Extensions.h"
#import "UIImage+compress.h"
#import "iflyMSC/IFlyFaceSDK.h"
#import "TTScanFaceDefine.h"
#import "TTCaptureManager.h"
#import "TTCanvasView.h"
#import "TTCalculatorTools.h"
#import "UIImage+Extensions.h"
#import "TTIFlyFaceImage.h"
#import "TTIFlyFaceResultKeys.h"

@interface TTFaceScanController ()<TTCaptureManagerDelegate>
{
    UILabel *alignLabel;
    int number;//
    int takePhotoNumber;
...
@property (nonatomic, retain ) TTCaptureManager             *TTCaptureManager;

@property (nonatomic, retain ) IFlyFaceDetector           *faceDetector;
@property (nonatomic, strong ) TTCanvasView                 *viewCanvas;
@property (nonatomic, strong ) UITapGestureRecognizer     *tapGesture;

@end

@implementation TTFaceScanController
@synthesize TTCaptureManager;
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    [self.TTCaptureManager removeObserver];
}
...
-(void)makeCamera
{
    ...
    self.TTCaptureManager=[[TTCaptureManager alloc] init];
    self.TTCaptureManager.delegate=self;
    
    self.previewLayer=self.TTCaptureManager.previewLayer;
    
    self.TTCaptureManager.previewLayer.frame= self.previewView.frame;
    self.TTCaptureManager.previewLayer.position=self.previewView.center;
    self.TTCaptureManager.previewLayer.videoGravity=AVLayerVideoGravityResizeAspectFill;
    [self.previewView.layer addSublayer:self.TTCaptureManager.previewLayer];
    
    self.viewCanvas = [[TTCanvasView alloc] initWithFrame:self.TTCaptureManager.previewLayer.frame] ;
    [self.previewView addSubview:self.viewCanvas] ;
    self.viewCanvas.center=self.TTCaptureManager.previewLayer.position;
    ...
    [self.TTCaptureManager.session addOutput:myStillImageOutput];
    
    //开始摄像
    [self.TTCaptureManager setup];
    [self.TTCaptureManager addObserver];
}

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    [super observeValueForKeyPath:keyPath ofObject:object change:change context:context];
    [self.TTCaptureManager observeValueForKeyPath:keyPath ofObject:object change:change context:context];
}
...
#pragma mark --- 脸部框识别
-(NSString*)praseDetect:(NSDictionary* )positionDic OrignImage:(TTIFlyFaceImage*)faceImg
{
    if(!positionDic){
        return nil;
    }
    ...
    // 判断摄像头方向
    BOOL isFrontCamera=self.TTCaptureManager.videoDeviceInput.device.position==AVCaptureDevicePositionFront;
...
#pragma mark --- 脸部部位识别
-(NSMutableArray*)praseAlign:(NSDictionary* )landmarkDic OrignImage:(TTIFlyFaceImage*)faceImg
{
    if(!landmarkDic){
        return nil;
    }
...
#pragma mark --- 脸部识别
-(void)praseTrackResult:(NSString*)result OrignImage:(TTIFlyFaceImage*)faceImg
{
    if(!result){
        return;
    }
...
#pragma mark - TTCaptureManagerDelegate
-(void)onOutputFaceImage:(TTIFlyFaceImage*)faceImg
{
