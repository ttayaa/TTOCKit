...
@interface TTCaptureManager ()<AVCaptureVideoDataOutputSampleBufferDelegate>

@end

@implementation TTCaptureManager
...
- (void)setup
{
    ...
    dispatch_async(sessionQueue, ^{
        ...
        NSError *error = nil;
        AVCaptureDevice *videoDevice = [TTCaptureManager deviceWithMediaType:AVMediaTypeVideo preferringPosition:AVCaptureDevicePositionFront];
...
- (void)addObserver
{
    dispatch_async([self sessionQueue], ^{
        ...
        __weak TTCaptureManager *weakSelf = self;
        [self setRuntimeErrorHandlingObserver:[[NSNotificationCenter defaultCenter] addObserverForName:AVCaptureSessionRuntimeErrorNotification object:self.session queue:nil usingBlock:^(NSNotification *note) {
            TTCaptureManager *strongSelf = weakSelf;
...
// Create a TTIFlyFaceImage from sample buffer data
- (TTIFlyFaceImage *) faceImageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer
{
    //获取灰度图像数据
    ...
    TTIFlyFaceImage* faceImage=[[TTIFlyFaceImage alloc] init];
    if(!faceImage){
        return nil;
    }
...
- (void)cameraToggle
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        AVCaptureDevice *videoDevice = [TTCaptureManager deviceWithMediaType:AVMediaTypeVideo preferringPosition:preferredPosition];
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(onOutputFaceImage:)]){
        TTIFlyFaceImage* faceImage=[self faceImageFromSampleBuffer:sampleBuffer];
...
{
    if([TTPermissionDetector isCapturePermissionGranted]){
